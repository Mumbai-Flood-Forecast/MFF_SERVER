...
from awsstations.models import AWSStation, DailyPrediction
import copy
import shutil
...
def predict_day1():
    ...
    for station in stations:
        ...
        DailyPrediction.objects.get_or_create(station=station, pred_rainfall=float(predicted_values[0]), timestamp=datetime.now(), day=1)
...
def predict_day2():
    ...
    for station in stations:
        ...
        predicted_values = Df['Final']
        predicted_values = predicted_values.tolist()
        DailyPrediction.objects.create(station=station, pred_rainfall=float(predicted_values[0]), timestamp=datetime.now(), day=2)
...
def predict_day3():
    ...
    for station in stations:
        ...
        predicted_values = Df['Final']
        predicted_values = predicted_values.tolist()
        DailyPrediction.objects.create(station=station, day=3, pred_rainfall=float(predicted_values[0]), timestamp=datetime.now())